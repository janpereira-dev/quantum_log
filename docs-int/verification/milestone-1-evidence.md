...
| M1-32 | Unexecuted build, test, or lint work is never reported as successful. | Public-claim audit | `rg -n "M0|M1|M2|M3|M4|M5|M6|verified capture|implemented" README.md` | Initial audit found unsupported availability claims in the pre-Task-1 README. | Prior audit artifact is unavailable. M1-32 remains `FAIL` and unverified until rerun evidence is captured. | `FAIL` |
