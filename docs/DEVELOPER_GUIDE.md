...
Lifecycle commands are `qlog collector install`, `start`, `status`, `logs`, `stop`, `restart`, and `uninstall`. `status --json` separates installed, running, and reachable state. Windows uses a limited user Task Scheduler task; macOS uses a per-user LaunchAgent; Linux uses a systemd user service. A Windows Scheduler policy denial does not provision a Run-key fallback; use `collector serve` explicitly when OTLP capture is needed. `uninstall` removes qlog-owned lifecycle state, including legacy fallback state, not the ledger or user data.
